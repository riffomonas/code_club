---
layout: post
title: "Adding lines and stars to indicate signficance to a faceted figure"
...
if(kt$p.value < 0.05){
  pt_invsimpson <- pairwise.wilcox.test(metadata_alpha$invsimpson,
...
if(kt$p.value < 0.05){
  pt_sobs <- pairwise.wilcox.test(metadata_alpha$sobs,
                       g=metadata_alpha$disease_stat,
                       p.adjust.method = "BH")
}
...
if(kt$p.value < 0.05){
  pt_shannon <- pairwise.wilcox.test(metadata_alpha$shannon,
                       g=metadata_alpha$disease_stat,
                       p.adjust.method = "BH")
}
...
#             aes(x=x, y=y),
#             inherit.aes=FALSE) +
#   geom_text(data=tibble(x=2.5, y=24.5),
#             aes(x=x, y=y, label="n.s."),
#             inherit.aes=FALSE) +
#   geom_text(data=tibble(x=1.75, y=33.25),
